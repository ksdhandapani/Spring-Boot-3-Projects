...
public class CustomerServiceImpl implements ICustomerService {
	...
	public CustomerDetailDto fetchCustomerDetailByMobileNumber(String mobileNumber, String correlationId) {
		...
		ResponseEntity<LoanDto> loanDtoResponseEntity = loansFeignClient.fetchLoanDetailByMobileNumber(correlationId,
				mobileNumber);
		if (null != loanDtoResponseEntity) {
			customerDetailDto.setLoanDto(loanDtoResponseEntity.getBody());
		}

		ResponseEntity<CardDto> cardDtoResponseEntity = cardsFeignClient.fetchCardByMobileNumber(correlationId,
				mobileNumber);
